"""
Configuration module for Multi-Model Trading Architecture.

Contains all hyperparameters, thresholds, and settings for:
    - Data processing
    - Model 1 (Screener): XGBoost classifier
    - Model 2 (Entry): LSTM/Hybrid entry timing
    - Model 3 (Exit): Exit point optimizer
    - Risk management rules
    - Backtesting parameters

Author: Trading Team
Date: 2025-12-29
"""

from dataclasses import dataclass, field
from typing import List, Dict, Any, Optional
import json
from pathlib import Path


# =============================================================================
# Data Configuration
# =============================================================================

# Dataset statistics from analysis
DATASET_SIZE = 956  # Total tokens
MEDIAN_LIFESPAN_SEC = 172
MEDIAN_TIME_TO_PEAK_SEC = 76
AVG_LIFESPAN_SEC = 220

# Data split ratios (chronological split)
TRAIN_SPLIT = 0.70
VAL_SPLIT = 0.15
TEST_SPLIT = 0.15

# Feature extraction windows
EARLY_WINDOW_SEC = 30  # First N seconds for screener features
SCREENER_DECISION_TIME = 30  # Time at which screener makes decision

# Minimum requirements
MIN_CANDLES_FOR_TRAINING = 10
MIN_CANDLES_FOR_SCREENER = 5


# =============================================================================
# Label Definitions
# =============================================================================

# Model 1: Screener Labels
SCREENER_WORTHY_THRESHOLD = 2.0  # 2x return to be considered worthy
SCREENER_LOOKAHEAD_SEC = 300  # 5 minutes lookahead

# Model 2: Entry Labels
ENTRY_PROFIT_THRESHOLD = 0.20  # 20% profit in next 60s for ENTER_NOW
ENTRY_MAX_DRAWDOWN = 0.15  # Max 15% drawdown for valid entry
ENTRY_LOOKAHEAD_SEC = 60  # 60 seconds lookahead

# Model 3: Exit Labels
EXIT_TRAILING_STOP_PCT = 0.15  # 15% from position high
EXIT_STOP_LOSS_PCT = 0.25  # 25% unrealized loss
EXIT_PROFIT_TARGET_PCT = 2.00  # 200% gain target
EXIT_TIME_LIMIT_SEC = 300  # 5 minute time limit
EXIT_GAIN_EXCEPTION_PCT = 1.00  # Exception for time limit if >100% gain


# =============================================================================
# Trading Parameters
# =============================================================================

# Position sizing
FIXED_POSITION_SIZE_SOL = 0.1  # SOL per trade
MAX_POSITION_PCT = 0.05  # Max 5% of portfolio per trade

# Fees (Solana/Jito)
JITO_TIP_SOL = 0.02
GAS_FEE_SOL = 0.003
PRIORITY_FEE_SOL = 0.001
TOTAL_FEE_PER_TX = JITO_TIP_SOL + GAS_FEE_SOL + PRIORITY_FEE_SOL  # ~0.024 SOL

# Slippage simulation
DELAY_SECONDS = 1  # Transaction confirmation delay


# =============================================================================
# Model 1: Screener Configuration (XGBoost)
# =============================================================================

@dataclass
class ScreenerConfig:
    """Configuration for Model 1: Entry Worthiness Screener (XGBoost)."""

    # Architecture
    n_estimators: int = 200
    max_depth: int = 6
    learning_rate: float = 0.05
    min_child_weight: int = 3
    subsample: float = 0.8
    colsample_bytree: float = 0.8
    gamma: float = 0.1
    reg_alpha: float = 0.1
    reg_lambda: float = 1.0

    # Class imbalance handling (AVOID is minority ~19%)
    scale_pos_weight: float = 4.0

    # Training
    early_stopping_rounds: int = 50
    eval_metric: str = "auc"
    random_state: int = 42

    # Decision
    decision_time_sec: int = SCREENER_DECISION_TIME
    confidence_threshold: float = 0.5

    # Features used (static + early dynamic)
    num_static_features: int = 7  # MC bin, KOL count, age, etc.
    num_dynamic_features: int = 12  # Returns, volume, tx count, etc.

    def to_xgb_params(self) -> Dict[str, Any]:
        """Convert to XGBoost parameter dictionary."""
        return {
            "n_estimators": self.n_estimators,
            "max_depth": self.max_depth,
            "learning_rate": self.learning_rate,
            "min_child_weight": self.min_child_weight,
            "subsample": self.subsample,
            "colsample_bytree": self.colsample_bytree,
            "gamma": self.gamma,
            "reg_alpha": self.reg_alpha,
            "reg_lambda": self.reg_lambda,
            "scale_pos_weight": self.scale_pos_weight,
            "random_state": self.random_state,
            "eval_metric": self.eval_metric,
            "use_label_encoder": False,
            "tree_method": "hist",  # Fast histogram-based
            "device": "cuda",  # GPU acceleration
        }


# =============================================================================
# Model 2: Entry Configuration (LSTM/Hybrid)
# =============================================================================

@dataclass
class EntryConfig:
    """Configuration for Model 2: Entry Timing Optimizer (LSTM/Hybrid)."""

    # Encoder architecture
    input_dim: int = 14  # Number of input features
    hidden_dim: int = 128
    num_layers: int = 2
    bidirectional: bool = True
    dropout: float = 0.2

    # Attention
    use_attention: bool = True
    attention_heads: int = 4

    # Output head
    head_hidden_dims: List[int] = field(default_factory=lambda: [64, 32])
    num_classes: int = 3  # ENTER_NOW, WAIT, ABORT

    # Training
    batch_size: int = 128
    learning_rate: float = 1e-3
    weight_decay: float = 1e-5
    warmup_epochs: int = 3
    total_epochs: int = 50
    patience: int = 10

    # Sequence
    max_seq_len: int = 120  # Max 2 minutes of data
    min_seq_len: int = 10

    # Decision
    confidence_threshold: float = 0.7
    max_wait_time_sec: int = 180  # 3 minutes max wait
    inference_interval_sec: int = 5  # Run every 5 seconds


# =============================================================================
# Model 3: Exit Configuration (Ensemble + Rules)
# =============================================================================

@dataclass
class ExitConfig:
    """Configuration for Model 3: Exit Point Optimizer."""

    # ML model architecture (XGBoost for speed)
    n_estimators: int = 100
    max_depth: int = 5
    learning_rate: float = 0.1

    # Training
    batch_size: int = 256

    # Output
    num_classes: int = 3  # EXIT_NOW, HOLD, PARTIAL_EXIT
    confidence_threshold: float = 0.6

    # Hard-coded risk management rules (override ML)
    stop_loss_pct: float = EXIT_STOP_LOSS_PCT  # 25%
    trailing_stop_pct: float = EXIT_TRAILING_STOP_PCT  # 15%
    time_stop_sec: int = EXIT_TIME_LIMIT_SEC  # 5 min
    profit_target_pct: float = EXIT_PROFIT_TARGET_PCT  # 200%
    time_exception_gain_pct: float = EXIT_GAIN_EXCEPTION_PCT  # 100%

    # Partial exit strategy
    partial_exit_threshold_pct: float = 0.30  # 30% gain
    partial_exit_fraction: float = 0.50  # Take 50% profit


# =============================================================================
# GPU Configuration
# =============================================================================

GPU_CONFIGS: Dict[str, Dict[str, int]] = {
    "A100": {"batch": 512, "hidden": 256, "lstm_layers": 3},
    "H100": {"batch": 512, "hidden": 256, "lstm_layers": 3},
    "L4": {"batch": 384, "hidden": 192, "lstm_layers": 2},
    "T4": {"batch": 256, "hidden": 128, "lstm_layers": 2},
    "CPU": {"batch": 64, "hidden": 64, "lstm_layers": 1},
}


def get_gpu_config(gpu_name: Optional[str] = None) -> Dict[str, int]:
    """
    Get GPU-specific configuration.

    Args:
        gpu_name: GPU name string (e.g., "NVIDIA A100"). If None, auto-detect.

    Returns:
        Dictionary with batch size and architecture settings.
    """
    if gpu_name is None:
        try:
            import torch
            if torch.cuda.is_available():
                gpu_name = torch.cuda.get_device_name(0).upper()
            else:
                return GPU_CONFIGS["CPU"]
        except ImportError:
            return GPU_CONFIGS["CPU"]

    for key in GPU_CONFIGS:
        if key in gpu_name.upper():
            return GPU_CONFIGS[key]

    return GPU_CONFIGS["T4"]  # Default


# =============================================================================
# Backtesting Configuration
# =============================================================================

@dataclass
class BacktestConfig:
    """Configuration for backtesting and evaluation."""

    # Initial capital
    initial_capital_sol: float = 10.0

    # Risk parameters
    max_position_pct: float = 0.05  # 5% per trade
    max_concurrent_trades: int = 3

    # Performance targets
    target_win_rate: float = 0.60  # 60%
    target_win_loss_ratio: float = 2.0  # 2:1
    target_max_drawdown: float = 0.30  # 30%

    # Simulation
    slippage_pct: float = 0.01  # 1% slippage
    include_fees: bool = True


# =============================================================================
# Feature Configuration
# =============================================================================

# Feature names for Model 1 (Screener)
SCREENER_STATIC_FEATURES = [
    "mc_bin_sub_5k",
    "mc_bin_5k_10k",
    "mc_bin_10k_15k",
    "mc_bin_15k_20k",
    "mc_bin_above_20k",
    "kol_count",
    "token_age_at_detection",
]

SCREENER_DYNAMIC_FEATURES = [
    "return_5s",
    "return_10s",
    "return_15s",
    "return_30s",
    "volume_10s",
    "volume_20s",
    "volume_30s",
    "tx_count_30s",
    "buy_sell_ratio_30s",
    "largest_tx_size",
    "volume_acceleration",
    "price_acceleration",
]

# Feature names for Models 2 & 3 (time-series)
TIMESERIES_FEATURES = [
    "log_close",
    "return_1s",
    "return_3s",
    "return_5s",
    "range_ratio",
    "volume_log",
    "rsi_norm",
    "macd_norm",
    "bb_upper_dev",
    "bb_lower_dev",
    "vwap_dev",
    "momentum_10s",
    "order_imbalance",
    "drawdown_from_high",
]


def save_config(config: Any, path: str) -> None:
    """Save configuration to JSON file."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)

    if hasattr(config, "__dataclass_fields__"):
        data = {k: getattr(config, k) for k in config.__dataclass_fields__}
    else:
        data = config

    with open(path, "w") as f:
        json.dump(data, f, indent=2, default=str)


def load_config(config_class: type, path: str) -> Any:
    """Load configuration from JSON file."""
    with open(path, "r") as f:
        data = json.load(f)
    return config_class(**data)


# Default configs
DEFAULT_SCREENER_CONFIG = ScreenerConfig()
DEFAULT_ENTRY_CONFIG = EntryConfig()
DEFAULT_EXIT_CONFIG = ExitConfig()
DEFAULT_BACKTEST_CONFIG = BacktestConfig()
