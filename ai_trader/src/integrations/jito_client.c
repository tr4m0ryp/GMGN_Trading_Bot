/**
 * @file jito_client.c
 * @brief Implementation of Jito Bundle client
 *
 * Uses CURL to submit JSON-RPC requests to Jito block engine.
 * Implements sendBundle and getBundleStatuses methods.
 *
 * JSON-RPC Endpoint: https://{region}.mainnet.block-engine.jito.wtf/api/v1/bundles
 *
...
int jito_submit_bundle(jito_client_t *client, const char **signed_txs, int tx_count,
    ...
    result->submit_time_ms = get_time_ms() - start_time;
        ...
        if (resp) {
            ...
            if (result_obj) {
                ...
                if (value && cJSON_IsArray(value) && cJSON_GetArraySize(value) > 0) {
                    ...
                    if (first) {
                        cJSON *confirmation_status = cJSON_GetObjectItemCaseSensitive(first, "confirmation_status");
                        if (confirmation_status && cJSON_IsString(confirmation_status)) {
                            const char *s = confirmation_status->valuestring;
                            if (strcmp(s, "confirmed") == 0 || strcmp(s, "finalized") == 0) {
