...
static void *tracker_thread(void *arg) {
    ...
    while (tracker->running) {
        ...
        for (int i = 0; i < TRACKER_MAX_TOKENS; i++) {
            ...
            /* Check if it's time to re-check this token (millisecond precision) */
            ...
            if (fetch_result == 0) {
                ...
                /* Track peak KOL count - once a KOL buys, keep it marked */
                ...
                /* Check if token now passes filters - USE PEAK KOL COUNT */
                bool mc_pass = filter_check_market_cap(info.market_cap,
                                                       tracker->filter);
                bool kol_pass = filter_check_kol(t->peak_kol_count,
                                                 tracker->filter);
...
int tracker_add_token(token_tracker_t *tracker, const pool_data_t *pool) {
    ...
    t->peak_kol_count = pool->base_token.kol_count; /* Initialize peak KOL */
